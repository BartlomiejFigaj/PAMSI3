...
int przekatna_lp(int i0, int j0, int dlugosc, int k, int gracz, int &najdluzszy, Plansza &plansza){
...
int przekatna_pl(int i0, int j0, int dlugosc, int k, int gracz, int &najdluzszy, Plansza &plansza){
  int dobre, puste, punkty=0;
  for(int i=i0, j=j0; dlugosc; --dlugosc, ++i, --j){
    dobre = puste = 0;
...
int punkty_heurystyczne(Plansza &plansza, int gracz){
  ...
  for(int i=0; i<n-k+1; ++i)
    punkty += przekatna_lp(i, 0, n-i, k, gracz, najdluzszy, plansza);
  for(int j=1; j<n-k+1; ++j)
    punkty += przekatna_lp(0, j, n-j, k, gracz, najdluzszy, plansza);
  for(int i=0; i<n-k+1; ++i)
    punkty += przekatna_pl(i, n-1, n-i, k, gracz, najdluzszy, plansza);
  for(int j=n-2; j>=k-1; --j)
    punkty += przekatna_pl(0, j, j+1, k, gracz, najdluzszy, plansza);
  punkty += 5*najdluzszy;
